...
select count(*) as [count of rows ] from customers 

--4.  Write a query to count the number of unique product categories (COUNT(DISTINCT Category)) from the Products table.

select (COUNT(DISTINCT Category)) as [ unique products name ] from Products
...
select count (EmployeeID) [count of emp] , DepartmentName from Employees
group by DepartmentName

--8.  Write a query to show the minimum and maximum Price of products grouped by Category. Use products table.

select*from Products
select min(price) as [MIN], Category from Products
group by Category
select max(price) [MAX], Category from Products
GROUP BY Category

--9.  Write a query to calculate the total (SUM) sales per Customer in the Sales table.

Select*from Sales
select sum (saleamount) [Sale Amount], CustomerID from Sales
group by CustomerID
...
select count (employeeId) [EMPID], DepartmentName from Employees
group by DepartmentName
...
select*from Sales
select sum (SaleAmount) [Sale Amount total], ProductID from Sales
group by ProductID

select avg (SaleAmount) [Sale Amount avg], ProductID from Sales
group by ProductID
...
select min(Salary) [Salary min], DepartmentName from Employees
group by DepartmentName

select max(Salary) [Salary max], DepartmentName from Employees
group by DepartmentName
...
select avg(Salary) [Salary avg], DepartmentName from Employees
group by DepartmentName
...
select avg (Price) [avg price], Category from Products
group by Category
...
select sum (SaleAmount) [Sum Sale Amount], SaleDate from Sales
group by SaleDate
...
select count (CustomerID) [numbers of customers] from Orders
group by CustomerID
...
Select sum (Salary) [Sum of Salary], DepartmentName from Employees
group by DepartmentName
...
select sum(SaleAmount) [Sum of Sale Amount], CustomerID from Sales   
Group by CustomerID
...
select sum (Salary) [Sum Salary], DepartmentName from Employees
group by DepartmentName

select avg (Salary) [Avg Salary], DepartmentName from Employees
group by DepartmentName
...
Select Max (TotalAmount) [Max], CustomerID from Orders
group by CustomerID
Having Max (TotalAmount) < 50

Select Min (TotalAmount) [Min], CustomerID from Orders
group by CustomerID
...
Select sum (SaleAmount) [SUM Amount], SaleDate from Sales
group by SaleDate

Select count ( ProductID) [ count of Products], SaleDate from Sales             
group by SaleDate
having count ( ProductID) > 8


--25. Write a query to find the MIN and MAX order quantity per Year. From orders table. (Do some research) Necessary tables:

select*from Orders

Select min (quantity) [min of Quantity], OrderDate from Orders
group by OrderDate

Select max (quantity) [max of Quantity], OrderDate from Orders
group by OrderDate
