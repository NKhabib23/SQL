...
select count (ProductName) [Count of Pr.Name], Category from Products
group by Category
...
select concat_ws(' ', FirstName,LastName) [Full Name],City from Customers
group by City, concat_ws(' ', FirstName,LastName)
having City like 'L%'

--4. Using Products table, get all product names that end with 'er'.
select ProductName from Products
group by ProductName
having ProductName like '%er'

--5. Using Customers table, list all customers from countries ending in 'A'.
select concat_ws(' ', FirstName, LastName) [Full name], Country from Customers
group By Country, concat_ws(' ', FirstName, LastName)
...
Select case when StockQuantity < 30 then 'Low Stock'
	   	    ...
	   	    end as [Stock Quantity], ProductName, Price, StockQuantity
from Products

--8 Using Customers table, find the total number of customers in each country.
select Country, count (CONCAT_WS(' ',FirstName, LastName)) [count of customers] from Customers
group by Country
...
Select year(orderDate) Order_Year, avg(TotalAmount) AvgTotalAmount from Orders
...
select*, case when Price<100 then 'Low'
			  when Price between 100 and 500 then 'Mid'
			  else 'High'
			end as Product_Groupped from Products
...
Pivot
(sum(Population)for year in ([2012],[2013]))as Pivoted_table
select*from Population_Each_Year

--16. Using Sales table, find total sales per product Id.

select*from Sales
Select ProductID, count (SaleID) as [Cnt_Sale], Sum (SaleAmount) as [Groupped_Sale_Amount] from Sales
group by ProductID
...
/*
18. Using City_Population table, use Pivot to show values of City column in seperate columns (Bektemir, Chilonzor, Yakkasaroy) 
and copy results to a new Population_Each_City table.*/

Select district_id, [Bektemir], [Chilonzor], [Yakkasaroy], [Mirobod], [Yashnobod], [Year] into Population_Each_City from city_population
Pivot
(
sum(Population) for District_name in ([Bektemir], [Chilonzor], [Yakkasaroy],[Mirobod], [Yashnobod])
) As PivotedTable
...
Select [District_name], [Population], [year] into CityPopulation from Population_Each_Year
unpivot
(
...
Select district_id, [District_name], [Population], [year] into City_Population1 from Population_Each_City
unpivot
(
Population for District_name in ([Bektemir], [Chilonzor], [Yakkasaroy], [Mirobod], [Yashnobod])
) as UnpivotedTable
Select*from City_Population1
